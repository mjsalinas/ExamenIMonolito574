...
public class CuidadoresController : ControllerBase
{
    ...
    {
        var cuidador = await _db.Cuidadores.FindAsync(id);
        var mascotas = await _db.Mascotas.AllAsync((Mascotas => Mascotas.CuidadorId == id));
        
        if (cuidador is null) return NotFound();

        if (mascotas) return Conflict("No se puede eliminar porque hay mascotas asignadas a este cuidador");
    ...
    [HttpGet("{id}/mascotas")]
    public async Task<IActionResult> obtenerMascotasCuidador(int id)
    {
        var cuidador = await _db.Cuidadores.FindAsync(id);
